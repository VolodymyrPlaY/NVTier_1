...
def is_valid_pin_codes(pin_codes):
    ...
    # Перевірка унікальності пін-кодів
    if len(set(pin_codes)) != len(pin_codes):
        return False

    # Перевірка кожного пін-коду на відповідність умовам
...
example_pin_codes = ['1101', '9034', '0011']
result = is_valid_pin_codes(example_pin_codes)
print("Валідність пін-кодів:", result)
# Ця функція перевіряє рядок пін-кодів на кілька умов:
#    Перевіряє, чи список не є порожнім.
#    Перевіряє унікальність пін-кодів у списку.
#    Перевіряє, чи кожний пін-код має довжину 4 та складається тільки з цифр.
#    Якщо усі перевірки пройдені, функція повертає True, інакше - False. 
